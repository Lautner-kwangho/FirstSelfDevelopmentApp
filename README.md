...
<br/><br/>
## 출시 후 회고
#### - Storyboard를 사용함으로써, UI를 직관적으로 확인할 수 있었으나 내용이 길어지거나 복잡한 로직을 추가 구현하고자 할 때 Xcode 무한 로딩 후 종료 이슈가 있었음. Source Code로 열어서 수정 및 업데이트 진행 다음번에는 Code UI를 사용하여 
#### - 지속적이 업데이트를 위해서 Magic Number를 줄이고 좀 더 직관적으로 네이밍을 진행해야 함. 또한 클린 코드에 중점을 두고 Swift 파일을 좀더 세부적으로 분류해야 함
#### - 기획을 할 때는 좀 더 테스트를 하고 화면의 재사용을 높여서 진행했어야 했음
#### - Enum을 이용하여 Text를 종합적으로 관리해야 함 유지 보수 할 때 일일이 찾아서 하기 힘듦
#### - HIG(Human Interface GuideLines)를 준수하여 사용자 입장에서 사용하기 편하게 구성해야겠다고 생각함
#### - SwiftLint를 처음부터 사용하여 코드 구성 제약을 했었다면 조금 더 탄탄하게 코드를 구성할 수 있지 않았을까 생각함
#### - 다음 앱 개발에서는 접근 제어자 사용 및 RC에 유념하여 앱을 구성했다면 Memory 관리 및 앱 부담감을 줄이기로
...
##### 1. 스크롤 버그
  ...
  - 한 배열 안에 덮어서 삭제 -> 추가 -> 로드하는 형식이었는데 가만히 생각해보면 임시배열을 하나 더 만들고 받을 데이터는 임시 배열에 추가, 이후 CollectionView에 로드하고 새로운 배열을 대체하는 건 Dispatchqueue?동기로 처리하면 오류가 발생하지 않을거 같은 생각만 해봤음 ( 추가로 동기 처리되는 동안 progress bar? 같은 거 보여주면서 조금 멈추더라도 정상적인거라고 알려주는 쪽으로)  
    ...
    <img width="300" alt="스크린샷 2021-11-30 오후 3 48 31" src="https://user-images.githubusercontent.com/80211277/144233675-377cac1a-56bb-41ad-8329-188e05993ea4.png">
...
##### 2. ScrollView & Zoom 
  ...
  - 다음은 subView에서만 줌 가능하다고 떴는데 졸린지 self.다른 걸 입력시켜놔서 오류가 난거라 이것도 빨리 해당 subView로 고쳐서 해결! 
    ![스크린샷 2021-12-13 오전 3 14 54](https://user-images.githubusercontent.com/80211277/145725733-d08b7fa0-00b4-49c5-93ed-a88ee9e88f52.png)
