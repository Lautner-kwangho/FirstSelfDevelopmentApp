...
class MainAPIManager {
    ...
    func mainCultureAPI(type: String, sort: String, place: String, realmtype: String ,fromDay: String, ToDay: String, result: @escaping (XMLIndexer) -> ()) {
        ...
        let url = "http://www.culture.go.kr/openapi/rest/publicperformancedisplays/\(type)?serviceKey=\(APIKEY.publicDataKey)&sortStdr=\(sort)&sido=\(placed)&realmCode=\(realmtype)&rows=200&from=\(fromDay)&to=\(ToDay)"
